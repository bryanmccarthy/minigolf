...
const courses = ["Practice", "Lazy Links", "Fairways"];
...
export default function Lobby() {
  ...
  const handleAcceptInvite = async (invite: Invite) => {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ party_id: invite.party_id })
      .eq("id", profile?.id)
      .select();
  ...
  useEffect(() => {
    ...
    const createParty = async () => {
      ...
      if (error) {
        ...
        if (data.length > 0) {
          ...
          const { data, error } = await supabase
            .from("parties")
            .insert([{ leader: profile?.id, game_state: "lobby" }])
            .select();
    ...
    const fetchInvites = async () => {
      const { data, error } = await supabase
        .from("invites")
        .select()
        .eq("receiver_display_name", profile?.display_name);
        ...
        <div className="relative flex">
          ...
          <div className="flex flex-col gap-2 p-2 w-48">
            <div className="flex flex-col">
              <div className="flex items-end gap-1">
                ...
                {partyMembers.length > 0 && (
                  <div
                    className="relative text-red-600 cursor-pointer"
                    onClick={handleToggleLeavePartyPane}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                    ...
                    {showConfirmLeaveParty && (
                      <div className="absolute flex justify-between items-center left-9 -top-1 w-44 h-10 px-2 rounded shadow-lg bg-white">
                        <button
                          className="text-light text-white bg-red-500 px-2 py-1 rounded"
              ...
              <div className="flex flex-col gap-2 py-2">
                <div className="flex items-center h-12 bg-slate-700 rounded shadow-lg">
                  <p className="w-40 text-white text-lg px-2 truncate">
                    {usernameEdit ? usernameEdit : profile.display_name} (Me)
                  </p>
                  {party?.leader === profile.id && (
                ...
                {partyMembers.map((member: Profile, idx: number) => (
                  <div
                    key={idx}
                    className="relative flex items-center h-12 bg-white rounded shadow-lg"
                  >
                    <p className="w-40 text-black text-lg px-2 truncate">
                      {member.display_name}
                    </p>
                    ...
                    {party?.leader === profile.id && (
                      <div
                        className="p-2 cursor-pointer transform hover:scale-110"
                        onClick={() => handleOpenPartyMemberEdit(member)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                    ...
                    {showPartyMemberEdit &&
                      ...
                      selectedMember.id === member.id && (
                        <PartyMemberEditPane
                          handleClosePartyMemberEdit={
                            handleClosePartyMemberEdit
                          }
                          handleMakeSelectedMemberLeader={
                            handleMakeSelectedMemberLeader
                          }
                          handleKickUserFromParty={() =>
                            handleKickUserFromParty(member.id)
                          }
            ...
            <div className="flex flex-col pt-2">
              <p className="text-2xl font-semibold text-black py-2">Course</p>
              <div className="relative flex flex-col gap-2">
                {party && profile.id === party.leader ? (
                  <>
                    <div
                      className="flex items-center justify-between h-12 bg-white rounded shadow-lg cursor-pointer"
                      onClick={handleShowCourseDropdown}
                    >
                      <p className="text-black text-lg px-2 truncate">
                        {party.course ? (
                          party.course
                        ) : (
                    ...
                    {party?.game_state === "lobby" && (
                      <button
                        className="flex items-center justify-center bg-orange-200 rounded h-12 shadow-lg"
                ...
                {party?.game_state === "game" && (
                  <button
                    className="flex items-center justify-center h-12 bg-orange-200 rounded shadow-lg"
                ...
                {party?.game_state === "game" &&
                  profile.id === party.leader && (
                    <button
                      className="flex items-center justify-center h-12 bg-red-400 rounded shadow-lg"
                ...
                {showCourseDropdown && (
                  <div className="absolute flex flex-col top-0 right-0 w-full rounded shadow-lg bg-white">
                    ...
                    <div className="flex flex-col gap-2 pb-2">
                      {courses
                        .filter((course: string) => course !== party?.course)
                        .map((course: string, idx: number) => (
                          <button
                            key={idx}
                            className="text-black text-lg text-start px-4 truncate bg-neutral-100 hover:bg-neutral-200 rounded mx-1"
          ...
          <div className="relative flex flex-col gap-2 p-2 ml-auto">
            <div className="flex items-center ml-auto gap-2">
              <button
                ...
                onClick={() => setShowPartyMessagesBox(!showPartyMessagesBox)}
              >
              ...
              <div
                className="flex justify-center items-center w-12 h-12 bg-white rounded shadow-lg cursor-pointer hover:bg-neutral-100"
            ...
            {party && showPartyMessagesBox && (
              <>
                ...
                <div className="flex gap-1 w-80">
                  <input
                    value={message}
                    className="w-full h-8 bg-white rounded shadow-lg outline-none px-1"
                    placeholder="message..."
                    onChange={(e) => handleMessageChange(e)}
                  />
                  <button
                    className="h-8 px-2 bg-blue-500 hover:bg-blue-600 text-white rounded shadow-lg"
              ...
              <div className="absolute flex flex-col top-0 right-0 w-56 m-2 rounded shadow-lg bg-white">
                ...
                <div className="flex flex-col gap-3 p-3 mt-auto">
                  <div className="flex items-center text-neutral-700 bg-neutral-100 px-2 py-1 rounded font-semibold truncate">
                    <input
                      className="w-40 bg-neutral-100 outline-none"
                      value={usernameEdit ? usernameEdit : profile.display_name}
                      onChange={(e) => handleUsernameInputChange(e)}
                    />
                    {showUsernameSave && (
                      <button
                        className="ml-auto h-5 w-5 rounded bg-neutral-800 text-white"
                  ...
                  <p className="text-neutral-700 bg-neutral-100 px-2 py-1 rounded font-semibold cursor-default truncate">
                    {session.user.email}
                  </p>
                  <button
                    className="text-red-500 px-2 py-1 text-start hover:text-red-600"
            ...
            {showNotifcations && (
              <div className="absolute flex flex-col top-0 right-0 w-56 max-h-96 m-2 rounded shadow-lg bg-white">
                <div className="flex justify-between items-center">
                  <p className="text-lg font-light text-black p-2">
                    Notifications
                  </p>
                  <button
                ...
                {invites.length > 0 ? (
                  ...
                  <div className="h-full">
                    <p className="grid place-items-center h-20 text-xl font-semibold text-neutral-500">
                      No notifications
                    </p>
                  </div>
                )}
